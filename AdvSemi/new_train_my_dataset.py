...
from dataset.my_dataset import VOCDataSet, VOCGTDataSet
import warnings
...
DATA_LIST_PATH = './dataset/my_dataset_list/dataset/train_aug.txt'
...
def make_D_label(label, ignore_mask):
    ...
    D_label = torch.from_numpy(D_label).float().cuda(args.gpu)
    # D_label = torch.from_numpy(D_label).float()
...
def main():
    ...
    if args.partial_data is None:
        ...
        if args.partial_id is not None:
            ...
            train_ids = list(range(train_dataset_size))  # 不打乱,前245个数据有标签
            print(train_ids)
        ...
        # trainloader_remain_iter = trainloader_remain


    trainloader_iter = enumerate(trainloader)
    trainloader_gt_iter = enumerate(trainloader_gt)
    # trainloader_iter = trainloader
    # trainloader_gt_iter = trainloader_gt
...
#                    _, batch = trainloader_remain_iter.next()
            ...
            pred = interp(model(images))
            # 默认参数
            out = torch.unique(labels)  
            print(out)  
            ###
            loss_seg = loss_calc(pred, labels, args.gpu)
            loss_seg = loss_seg/args.iter_size/2
            ...
            # 分割网络的loss曲线
            # print(loss_seg_value)
            train_loss_seg.append(loss_seg_value)
            ...
            # D_gt_v = Variable(one_hot(labels_gt))
            ...
            # 判别网络的loss曲线
            # print(loss_D_value)
            train_loss_D.append(loss_D_value)
    ...
    with open("./train_loss_seg.txt", 'w') as train_los_seg:
        train_los_seg.write(str(train_loss_seg))
    with open("./train_loss_D.txt", 'w') as train_lo_D:
        train_lo_D.write(str(train_loss_D))
