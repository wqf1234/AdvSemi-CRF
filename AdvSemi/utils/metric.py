...
class ConfusionMatrix(object):
    ...
    def recall(self):
        ...
        for i in range(self.nclass):
            if not self.M[i, i] == 0:
                recall_perclass.append(self.M[i, i] / np.sum(self.M[:, i]))
            # recall += self.M[i, i] / np.sum(self.M[:, i])

        #return recall/self.nclass
        return np.sum(recall_perclass)/len(recall_perclass), recall_perclass, self.M
    ...
    def accuracy(self):
        ...
        for i in range(self.nclass):
            if not self.M[i, i] == 0:
                accuracy_perclass.append(self.M[i, i] / np.sum(self.M[i, :]))

        # return accuracy/self.nclass
        return np.sum(accuracy_perclass)/len(accuracy_perclass), accuracy_perclass, self.M
